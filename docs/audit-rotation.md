...
**T1-02 — Public endpoints and data exposure.** Baseline: `none`; due now
...
| T1-02 | Public endpoints and data exposure | `backend/public.go`, `backend/ops.go`, `backend/ratelimit.go`, `backend/nodes.go`, `backend/runs.go`, `backend/geoip.go`, `backend/rdap.go`, `backend/speedtest.go`, `backend/alerts.go`, `backend/main.go`, `backend/client_ws.go`, `frontend/src/components/GeoMap.jsx` | Anonymous access, target/command restrictions, metrics and shared-run disclosure, outbound request boundaries, rate limits and HTML escaping. | none | none | due — first pass |
| T1-03 | Command execution and interactive shells | `agent/main.go`, `agent/commands.go`, `agent/shell.go`, `backend/main.go`, `backend/client_ws.go`, `backend/command_cleanup.go`, `backend/scheduler.go`, `frontend/src/components/ShellTerminal.jsx` | Allowlists, argv/options validation, shell authorization and opt-out, privileges, terminal input/output limits, cancellation and process cleanup. | none | none | due — first pass |
| T1-04 | Secrets and publication privacy | `scripts/check-privacy.py`, `scripts/check-commit-privacy.py`, `scripts/install-hooks.py`, `scripts/tests/`, `.githooks/`, `.gitignore`, `.dockerignore`, `.github/workflows/`, `backend/main.go`, `backend/middleware.go`, `agent/main.go`, `deploy/`, `tests/integration/` | Secret/config handling and log redaction; tracked files, commit metadata, package/image contents and CI artifacts must not disclose credentials or deployment identities. | none | none | due — first pass |
